...
use super::{OPCODE_SIZE, RAM_SIZE, REGISTER_SIZE, STACK_SIZE, FONT, DISPLAY_HEIGHT, DISPLAY_WIDTH};
...
impl Processor {
    ...
    pub fn execute(&mut self, opcode: u16) -> Output {
        ...
        let pc_change: ProgramCounter = match nibbles {
            ...
            (0x0d, _, _, _) => {
                ...
                self.ram[Register::VF as usize] = 0;
                for byte in 0..n {
                    let y = (self.registers[y] as usize + byte) % DISPLAY_HEIGHT;
                    for bit in 0..8 {
                        let x = (self.registers[x] as usize + bit) % DISPLAY_WIDTH;
                        let new_state = ((self.ram[self.i_reg as usize + byte] >> (7 - bit)) & 1) == 1;
                        let old_state = self.display.get_pixel(y, x);
                        let mut bit_op: usize = if old_state {1} else {0};
                        bit_op ^= if new_state {1} else {0};
                        self.registers[Register::VF as usize] = if old_state && !new_state {1} else {0};
                        self.display.set_pixel(y, x, bit_op == 1);
                        pixel_y.push(y);
                        pixel_x.push(x);
        ...
        match pc_change {
            ...
            ProgramCounter::Block => self.pc -= 2 * OPCODE_SIZE as u16,
    ...
    pub fn load_data(&mut self, data: Option<Box<[u8]>>) -> usize {
        self.reset();
        let mut start = 0;
        self.pc = 0x200;
        if let Some(d) = data {
